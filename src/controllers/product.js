...
const createProductByAdmin = async (req, res) => {
  try {
    ...
    // ✅ [NEW BLOCK] Optional: Validate tierPrices format
    if (body.tierPrices) {
      if (!Array.isArray(body.tierPrices)) {
        return res.status(400).json({
          success: false,
          message: 'tierPrices must be an array',
        });
      }

      for (const tier of body.tierPrices) {
        if (
          ...
          return res.status(400).json({
            success: false,
            message: 'Each tier must include numeric minQty and pricePerUnit',
          });
        }
      }
    }
